...
function StatBar({ label, v, accent }: { label: string; v: number; accent: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="w-8 shrink-0 font-mono text-[9px] tabular-nums text-ink-dim">
        {Math.round(v * 100)}%
      </span>
    </div>
  );
        ...
        {CHARACTER_ORDER.map((id) => {
          ...
          return (
            <motion.button
              ...
              disabled={!c.unlocked}
              className={`flex flex-col rounded-xl border px-2.5 pb-2.5 pt-2 text-left transition-colors ${
                sel
                  ? ""
                  : c.unlocked
              ...
              style={
                sel
                  ? {
                      ...
                      background: `${c.colors.band}1a`,
                    }
                  : undefined
              }
            >
              {/* emblem: flat weapon glyph in the identity color */}
              <span
                className="grid h-10 w-full place-items-center text-2xl leading-none"
                style={{ color: c.unlocked ? c.colors.band : "#4b5563" }}
              >
      ...
      <div className="mt-2 grid grid-cols-[240px_1fr] gap-2 max-md:grid-cols-1">
        <div
          ...
          style={{
            ...
            background: `radial-gradient(ellipse 150% 110% at 50% 28%, ${accent}28 0%, ${accent}1c 48%, rgba(14,20,40,0.9) 90%)`,
          }}
        >
        ...
        <div className="rounded-2xl border border-white/10 bg-space/85 px-4 py-3">
          {/* hierarchy: the REASON to pick them first — stats last */}
          <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-ink-dim/70">
            ability
          </p>
          ...
          <p className="mt-2 font-mono text-[9px] uppercase tracking-[0.2em] text-ink-dim/70">
            passive
          </p>
          ...
          <div className="mt-1 space-y-1">
            <StatBar label="vitality" v={ch.hp} accent={accent} />
            <StatBar label="damage" v={ch.dmg} accent={accent} />
            <StatBar label="speed" v={ch.speed} accent={accent} />
            <StatBar label="fire rate" v={Math.max(0.2, 2 - ch.cooldown)} accent={accent} />
            <StatBar label="fortune" v={ch.luck} accent={accent} />
          </div>
          <p className="mt-2 font-mono text-[9px] uppercase tracking-[0.2em] text-ink-dim/70">
            best for: {ch.bestFor}
          </p>
